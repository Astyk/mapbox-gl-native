...
void setResourceTransform(JNIEnv *env, jni::jobject* obj, jlong defaultFileSourcePtr, jni::jobject* transformCallback) {
    ...
    if (transformCallback) {
        ...
        defaultFileSource->setResourceTransform([
            ...
            mbgl::Log::Warning(mbgl::Event::JNI, "transforming url %s", resource.url.c_str());
            auto url = std_string_to_jstring(env, resource.url);
            url = reinterpret_cast<jni::jstring*>(
                jni::CallMethod<jni::jobject*>(*env, callback.get(), *transformOnURLMethodId, int(resource.kind), url));
            resource.url = std_string_from_jstring(env, url);
